...
- **Developer Playtest & Economy Tracking panel** (Town Hall dev section, collapsed by default): per-session analytics — session start/duration/start-level/start-restoration; first-event times (scrap/component/finished good/contract/upgrade/L2) shown as "Not reached" until they occur; economy totals (coins earned, milestone coins, coins spent, net, XP, restoration, scrap collected, components/finished produced); contract activity (totals + by tier + refreshes + avg per contract); production (storage-full count, machine idle/active, slot utilization %, jobs by recipe); milestone unlock times + current reward buff. Controls: Copy JSON (versioned), Copy CSV (RFC-4180 escaped, one metric/row, "Not reached" for blanks), Reset Tracking (confirm; analytics-only, preserves all game progress), Reset Game (confirm). expo-clipboard with graceful error toast. Verified by testing agent iter 5 (22/22 pass).
